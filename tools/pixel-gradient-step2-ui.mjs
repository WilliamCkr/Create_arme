...
function runRetexture(payload) {
  return new Promise(resolve => {
    const edgeBandPx = Number(payload.edgeBandPx ?? 18);
    const sourceInsetPx = Number(payload.sourceInsetPx ?? 2);
    const gradientSpanPx = Number(payload.gradientSpanPx ?? 28);
...
<div class="layout">
  <aside class="panel">
    ...
    <div class="grid">
      ...
      <input id="edgeBandPx" type="number" step="1" value="18">

      <label>Source inset px</label>
      <input id="sourceInsetPx" type="number" step="1" value="2">

      <label>Gradient span px</label>
      <input id="gradientSpanPx" type="number" step="1" value="28">
    </div>

    <h2>Presets</h2>
    <button onclick="preset(10,0,40)">Plus violet</button>
    <button onclick="preset(18,2,28)">Equilibre</button>
    <button onclick="preset(28,4,36)">Plus metal</button>
